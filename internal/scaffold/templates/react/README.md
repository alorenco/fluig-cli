...
Widget Fluig criada com `fluigcli widget new --template react`: uma SPA
**React 19 + TypeScript + Vite** dentro da casca oficial de widget. Para o
servidor Fluig ela é uma widget comum (WAR com `application.info` + FTLs);
para você é um projeto React moderno com hot reload e build de um comando.
...
├── src/react/                 ← o código da SPA — NÃO vai pro WAR
│   ├── main.tsx             ← ponte SuperWidget ↔ React (monta 1 root por instância)
│   ├── App.tsx              ← componente raiz (exemplo com style guide + dataset)
│   ├── hooks/useDataset.ts
...
nenhuma credencial fica em arquivo. Salvou um `.tsx`, a tela atualiza na hora.
...
clássico, sem React). O botão Salvar envia os campos nomeados via
...
`App.tsx`. Para criar uma preferência nova: acrescente o campo no `edit.ftl`
e leia `configs.<nome>` no App (exemplo: `customTitle`).
...
- O que a **SPA** mostra usa `src/react/fluig/i18n.ts` (o idioma vem de
...
próprios ficam no `app.css` — o React não tem CSS com escopo automático,
então **sempre prefixe os seletores** com o container da widget
(`[id^="[[.CamelCode]]_"] ...`) para não vazar para o portal.

## Regras de ouro

- A widget pode aparecer **mais de uma vez por página** — nunca use id fixo
  no DOM; o `main.tsx` já monta um root por `instanceId`. Nos FTLs, todo id
  leva `${instanceId}`.
