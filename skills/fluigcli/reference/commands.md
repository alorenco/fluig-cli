...
| `request start <processId> [--fields-file arq.json\|-] [--field k=v]... [--attach arq] [--target-state N] [--assignee login] [--comment s] [--no-send]` | inicia solicitação; --fields-file = objeto JSON plano (use `-` para stdin — o modo natural para agentes; --field sobrepõe); **linha de tabela-filha = sufixo `campo___N`** (3 underscores, N começa em 1) no mesmo JSON plano — é assim que se testa processo com anexo em tabela; com --attach usa SOAP (a REST não sobe anexo) e requer --target-state; throw de evento vira exit 5 com a mensagem. ⚠️ `--assignee` para atividade de POOL = recusa "Usuário selecionado não encontrado" (a CLI explica e manda omitir a flag — pool recebe o pool, não pessoa). ⚠️ Os eventos do FORMULÁRIO (displayFields/validateForm) NÃO rodam — o card é gravado pela REST e uma solicitação com os obrigatórios vazios é ACEITA sem crítica; não sirva de teste de validação de formulário |
...
| comando | efeito |
|---|---|
| `document list [<folderId>] [--recursive] [--depth N]` | sem arg = pastas raiz; com id = conteúdo (pastas/arquivos/artigos). `--recursive` desce a árvore (default 10 níveis) mostrando o CAMINHO de cada item — a verificação pós-deploy "foi para a pasta certa?" em 1 chamada; teto de 300 pastas com AVISO quando trunca  `id` e `documentId` são sinônimos no envelope (idem no `form list`) |
