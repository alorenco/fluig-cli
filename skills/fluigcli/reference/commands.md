...
| `dataset query <id> [--fields a,b]` | — | consulta os dados de um dataset (`--order` aceita um único campo; sufixo `_DESC`). `--fields` recorta no cliente, na ordem pedida — dataset customizado ignora o pedido e devolve tudo; campo inexistente vira aviso no stderr, não erro |
